...
class RscpFrame {
	...
	appendData( tag, value ) {
		var type = parseInt( rscpTags[tag].DataTypeHex, 16 );
		var buf2 = Buffer.alloc(2);
		var buf4 = Buffer.alloc(4);
		var buf6 = Buffer.alloc(6);
		...
		switch( type ) {
			...
			case TYPE_RSCP_TIMESTAMP: // CAUTION: treating value as seconds - setting nanoseconds to zero
				...
				this.buffer = Buffer.concat( [this.buffer, buf6, [0x00,0x00,0x00,0x00]] );
			default:
...
function display( f ) {
	function displayData ( f, pos ) {
		var tag = f.readUInt32LE(pos);
		var type = f.readUInt8(pos+4);
		var len = f.readUInt16LE(pos+5);
		...
		switch( type ) {
			...
			case TYPE_RSCP_UCHAR8:
			case TYPE_RSCP_BOOL:
				var c = f.readUInt8(pos+7);
				if( c > 31 && c < 127 && (type == TYPE_RSCP_CHAR8 || type == TYPE_RSCP_UCHAR8)  ) {
					console.log( "DATA VALUE: " + f.toString('utf8',pos+7,pos+8) );
				} else {
					console.log( "DATA VALUE: 0x" + c.toString(16).toUpperCase().padStart(2,"0") );
				}
				return 7+len;
			case TYPE_RSCP_INT16:
	...
	var magic = f.toString('hex',0,2).toUpperCase();
	if( magic == 'E3DC' ) {
		console.log( "MAGIC: >" + magic + "< is OK");
	} else {
		console.log( "MAGIC: >" + magic + "< is WRONG");
	}
	var ctrl = f.toString('hex',2,4).toUpperCase();
	switch( ctrl ) {
		case '0010':
