...
class E3dcRscp extends utils.Adapter {
                        ...
                        this.getState(`${prefix}.START_MINUTE`, (err, startMinute) => {
                            this.getState(`${prefix}.END_HOUR`, (err, endHour) => {
                                this.getState(`${prefix}.END_MINUTE`, (err, endMinute) => {
                                    ...
                                    this.addTagtoFrame(
                                        'TAG_EMS_IDLE_PERIOD_MINUTE',
                                        '',
                                        startMinute ? startMinute.val : 0,
                                    );
                                ...
                                this.getState(`${node}.PERIOD_START`, (err, start) => {
                                    this.getState(`${node}.PERIOD_STOP`, (err, stop) => {
                                        this.getState(`${node}.IDLE_PERIOD_TYPE`, (err, type) => {
                                            this.log.silly(
                                                `queueSetIdlePeriod2(): ${start ? start.val : '00:00:00'}-${stop ? stop.val : '00:00:00'} translates to ${helper.timeOfDayStringToSeconds(start ? start.val : '00:00:00')}-${helper.timeOfDayStringToSeconds(stop ? stop.val : '00:00:00')}`,
                                            );
                                            const c2 = this.startContainer('TAG_EMS_IDLE_PERIOD_2');
                                            this.addTagtoFrame('TAG_EMS_PERIOD_ACTIVE', '', active ? active.val : 0);
                                            this.addTagtoFrame(
                                                'TAG_EMS_PERIOD_DESCRIPTION',
                                                '',
                                                description ? description.val : '',
                                            );
                                            this.addTagtoFrame(
                                                'TAG_EMS_PERIOD_WEEKDAYS',
                                                '',
                                                helper.weekdayStringToBitmask(weekdays ? weekdays.val : 0),
                                            );
                                            this.addTagtoFrame(
                                                'TAG_EMS_PERIOD_START',
                                                '',
                                                helper.timeOfDayStringToSeconds(start ? start.val : '00:00:00'),
                                            );
                                            this.addTagtoFrame(
                                                'TAG_EMS_PERIOD_STOP',
                                                '',
                                                helper.timeOfDayStringToSeconds(stop ? stop.val : '00:00:00'),
                                            );
                                            this.addTagtoFrame('TAG_EMS_IDLE_PERIOD_TYPE', '', type ? type.val : 0);
                                            this.endContainer(c2);
                                            if (--counter < 0) {
                                                // async detect loop work finished
                                                this.pushFrame(c1);
                                                this.clearFrame();
    ...
    queueGetHistoryData(globalId) {
        ...
        const el = globalId.split('.');
        if (el.length == 5) {
            // e.g. "e3dc-rscp.0.DB.HISTORY_DATA_DAY.TIME_START"
                    ...
                    this.getState(`${nameSpace}.${shortTag}.TIME_START`, (err, timeStart) => {
                        this.getState(`${nameSpace}.${shortTag}.TIME_INTERVAL`, (err, interval) => {
                            this.getState(`${nameSpace}.${shortTag}.TIME_SPAN`, (err, span) => {
                                ...
                                this.pushFrame();
                                this.log.debug(
                                    `TAG_DB_REQ_${shortTag} - START=${d.getTime() / 1000} INTERVAL=${i} SPAN=${s}`,
                                );
                            });
                        });
                    });
    ...
    requestAllData(sml) {
        ...
        if (this.config.query_pm) {
            this.queuePmRequestData(sml);
        }
        if (this.config.query_ep) {
            this.queueEpRequestData(sml);
        }
        if (this.config.query_bat) {
            this.queueBatRequestData(sml);
        }
        if (this.config.query_dcdc) {
            this.queueDcdcRequestData(sml);
        }
        if (this.config.query_pvi) {
            this.queuePviRequestData(sml);
        }
        if (this.config.query_sys) {
            this.queueSysRequestData(sml);
        }
        if (this.config.query_info) {
            this.queueInfoRequestData(sml);
        }
        this.log.silly(`maxIndex["WB"] = ${this.maxIndex['WB']}`);
        if (this.config.query_wb) {
            wb.queueWbRequestData(sml);
        }
        this.sendFrameFIFO();
    }
    ...
    sendFrame(f) {
        if (f) {
            ...
            if (this.encryptionIV) {
                encryptedFrame.copy(this.encryptionIV, 0, encryptedFrame.length - BLOCK_SIZE);
            }
    ...
    parseTlv(buffer, start, end) {
        ...
        while (start < end) {
            ...
            if (!rscpTag[tagCode]) {
                this.log.debug(
                    `Unknown tag: tagCode=0x${tagCode.toString(16)}, len=${len}, typeCode=0x${typeCode.toString(16)}`,
                );
                ...
                switch (typeName) {
                    ...
                    case 'Timestamp':
                        ...
                        value = helper.dateToString(
                            new Date(
                                (Number(buffer.readBigUInt64LE(start + 7)) +
                                    buffer.readUInt32LE(start + 7 + 8) / 1000000) *
                                    1000,
                            ),
                        );
                        break;
                    ...
                    case 'None':
                        if (len > 0) {
                            this.log.warn(
                                `Received data type NONE with data length = ${len} - tagCode 0x${tagCode.toString(16)}`,
                            );
                        }
                        break;
                    default:
    ...
    processFrame(buffer) {
        ...
        if (buffer.length < 18 + dataLength + (hasCrc ? 4 : 0)) {
            this.log.debug(
                `Received message with inconsistent length: ${buffer.length} vs ${18 + dataLength + (hasCrc ? 4 : 0)}`,
            );
            ...
            if (hasCrc && CRC32.buf(buffer.slice(0, 18 + dataLength)) != buffer.readInt32LE(18 + dataLength)) {
                this.log.debug(
                    `Received message with invalid CRC-32: 0x${CRC32.buf(buffer.slice(0, 18 + dataLength)).toString(16)} vs 0x${buffer.readUInt32LE(18 + dataLength).toString(16)} - dataLength = ${dataLength}`,
                );
                this.log.silly(dumpRscpFrame(buffer));
            } else {
    ...
    processTree(tree, path) {
        ...
        for (const i in tree) {
            ...
            if (typeName == 'Error') {
                ...
                if (
                    ['PM.DEVICE_STATE', 'PM.REQ_DEVICE_STATE'].includes(shortId) &&
                    ['RSCP_ERR_NOT_AVAILABLE', 'RSCP_ERR_OUT_OF_BOUNDS'].includes(rscpError[token.content])
                ) {
                ...
                if (!ignoreIds.includes(shortId)) {
                    this.log.warn(
                        `Received data type ERROR: ${rscpError[token.content]} (${token.content}) - tag ${rscpTag[token.tag].TagNameGlobal} (0x${token.tag.toString(16)})`,
                    );
                    continue;
                }
            }
            if (typeName == 'Container') {
                if (shortId == 'EMS.SYS_SPEC' && token.content.length == 3) {
                    this.storeValue(
                        ...
                        token.content[1].content,
                        sysSpecUnits[token.content[1].content],
                    );
                    this.extendObject(`EMS.SYS_SPECS`, { type: 'channel', common: { role: 'info' } });
                } else if (
                    shortId == 'PVI.DATA' &&
                    token.content.length == 2 &&
                    rscpType[token.content[1].type] == 'Error' &&
                    rscpError[token.content[1].content] == 'RSCP_ERR_FORMAT'
                ) {
                ...
                if (multipleValueIds.includes(shortId)) {
                    ...
                    if (tagName == 'DCB_CELL_TEMPERATURE') {
                        if (token.content > 4.5) {
                            ...
                            // low values are regarded as voltage (seems to be a flaw in RSCP interface)
                            unit = 'V';
                    ...
                    if (tagName == 'DCB_CELL_TEMPERATURE' && v == 0) {
                        v = null;
                    ...
                    let r = 'info';
                    if (tagName.includes('TEMPERATURE')) {
                ...
                if (!notIndexIds.includes(shortId)) {
                    ...
                    if (tagName.endsWith('_COUNT')) {
                        if (this.maxIndex[`${pathNew}${tagName.replace('_COUNT', '')}`] != token.content - 1) {
                            ...
                            this.log.debug(
                                `Adjusted ${pathNew}${tagName.replace('_COUNT', '')} max. index to ${token.content - 1}`,
                            );
                        }
                    }
                }
                ...
                if (shortId == 'EMS.DPP_MONTHS_ACTIVE') {
                    this.storeValue(
                        ...
                        'DPP_MONTHS_ACTIVE',
                        'CString',
                        helper.bitmaskToMonthString(token.content),
                        'DPP_MONTHS_ACTIVE',
                    );
                    continue;
                }
                ...
                if (shortId == 'EMS.START_EMERGENCY_POWER_TEST' && token.content > 0) {
                    this.getState('EMS.START_EMERGENCY_POWER_TEST', (_err, _obj) => {
                ...
                if (mapReceivedIdToState[shortId]) {
                    ...
                    if (mapReceivedIdToState[shortId][typeName]) {
                        targetStateMatch = typeName;
                    }
                    if (
                        targetStateMatch &&
                        mapReceivedIdToState[shortId][targetStateMatch].targetState == 'RETURN_CODE' &&
                        token.content < 0
                    ) {
                ...
                if (targetStateMatch) {
                    tagNameNew = mapReceivedIdToState[shortId][targetStateMatch].split('.')[1];
                }
    ...
    adjustTypeAndValue(shortId, typeName, value) {
        ...
        if (negateValueIds.includes(shortId)) {
            newValue = -newValue;
        }
        if (percentValueIds.includes(shortId)) {
    ...
    storeIdlePeriods(tree, path) {
        if (this.config.periods_v1) {
            tree.forEach(token => {
                if (rscpTag[token.tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD' || token.content.length != 5) {
                    return;
                }
                if (rscpTag[token.content[0].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_TYPE') {
                    return;
                }
                const type = token.content[0].content;
                if (rscpTag[token.content[1].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_DAY') {
                    return;
                }
                ...
                if (!this.sendTupleTimeout[`EMS.${idleNode}.${dayNode}`]) {
                    // do not overwrite manual changes which are waiting to be sent
                    if (rscpTag[token.content[2].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_ACTIVE') {
                        return;
                    }
                    const active = token.content[2].content;
                    if (
                        rscpTag[token.content[3].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_START' ||
                        token.content[3].content.length != 2
                    ) {
                        return;
                    }
                    if (rscpTag[token.content[3].content[0].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_HOUR') {
                        return;
                    }
                    const startHour = token.content[3].content[0].content;
                    if (rscpTag[token.content[3].content[1].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_MINUTE') {
                        return;
                    }
                    const startMinute = token.content[3].content[1].content;
                    if (
                        rscpTag[token.content[4].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_END' ||
                        token.content[4].content.length != 2
                    ) {
                        return;
                    }
                    if (rscpTag[token.content[4].content[0].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_HOUR') {
                        return;
                    }
                    const endHour = token.content[4].content[0].content;
                    if (rscpTag[token.content[4].content[1].tag].TagNameGlobal != 'TAG_EMS_IDLE_PERIOD_MINUTE') {
                        return;
                    }
                    ...
                    this.extendObject(`EMS.${newPath.slice(0, -1)}`, {
                        type: 'channel',
                        common: { role: 'calendar.day' },
                    });
                }
            });
                ...
                tree.forEach(token => {
                    if (rscpTag[token.tag].TagNameGlobal == 'TAG_EMS_IDLE_PERIOD_2') {
                        ...
                        token.content.forEach(member => {
                            switch (rscpTag[member.tag].TagNameGlobal) {
                                case 'TAG_EMS_PERIOD_ACTIVE':
                                    ...
                                    break;
                                case 'TAG_EMS_PERIOD_DESCRIPTION':
                                    desc = member.content;
                                    break;
                                case 'TAG_EMS_PERIOD_WEEKDAYS':
                                    weekdays = member.content;
                                    break;
                                ...
                                default:
                                    this.log.debug(
                                        `storeIdlePeriods2: got unexpected tag ${rscpTag[member.tag].TagNameGlobal}`,
                                    );
                            }
                        });
                        ...
                        this.storeValue(
                            ...
                            'PERIOD_WEEKDAYS',
                            'CString',
                            helper.bitmaskToWeekdayString(weekdays),
                            'PERIOD_WEEKDAYS',
                        );
                        this.storeValue(
                            'EMS',
                            ...
                            'PERIOD_START',
                            'CString',
                            helper.secondsToTimeOfDayString(start),
                            'PERIOD_START',
                        );
                        this.storeValue(
                            'EMS',
                            ...
                            'PERIOD_STOP',
                            'CString',
                            helper.secondsToTimeOfDayString(stop),
                            'PERIOD_STOP',
                        );
    ...
    storeHistoryData(tree, path) {
        this.getState(`DB.${path}TIME_START`, (err, timeStartObj) => {
            this.getState(`DB.${path}TIME_INTERVAL`, (err, intervalObj) => {
                this.getState(`DB.${path}TIME_SPAN`, (_err, _spanObj) => {
                    let timeStart = 0;
                    if (timeStartObj && timeStartObj.val) {
                        timeStart = helper.stringToDate(timeStartObj.val.toString()).getTime() / 1000;
                    } // epoch seconds
                    let interval = 0;
                    if (intervalObj && intervalObj.val) {
                        interval = Number(intervalObj.val);
                    }
                    // let span = 0;
                    // if (spanObj && spanObj.val) {
                    //    span = Number(spanObj.val);
                    // }
                    let count = 0;
                    let oPath = '';
                    let oRole = '';
                    ...
                    tree.forEach(token => {
                        if (rscpTag[token.tag].TagNameGlobal == 'TAG_DB_SUM_CONTAINER') {
                            ...
                            this.storeValue(
                                ...
                                'TIMESTAMP',
                                'CString',
                                helper.dateToString(timeStamp),
                                'TIMESTAMP',
                            );
                        } else {
                            this.log.debug(
                                `storeHistoryData: ignoring unexpected tag ${rscpTag[token.tag].TagNameGlobal}`,
                            );
                            return; // next token
                        }
    ...
    clearAllIntervals() {
        if (this.dataPollingTimerS) {
            clearInterval(this.dataPollingTimerS);
        }
        if (this.dataPollingTimerM) {
            clearInterval(this.dataPollingTimerM);
        }
        if (this.dataPollingTimerL) {
            clearInterval(this.dataPollingTimerL);
        }
        if (this.setPowerTimer) {
            clearInterval(this.setPowerTimer);
        }
        if (this.checkAuthTimeout) {
            clearInterval(this.checkAuthTimeout);
        }
        for (const [, timeout] of Object.entries(this.sendTupleTimeout)) {
            if (timeout) {
    ...
    async onReady() {
        ...
        this.log.debug(
            `config.*: (${this.config.e3dc_ip}, ${this.config.e3dc_port}, ${this.config.portal_user}, ${this.config.polling_interval_short}, ${this.config.polling_interval_medium}, ${this.config.polling_interval_long}, ${this.config.setpower_interval})`,
        );
        this.initChannel();

        // In order to get state updates, you need to subscribe to them. The following line adds a subscription for our variable we have created above.
        this.subscribeStates('RSCP.AUTHENTICATION');
        for (const s in mapChangedIdToSetTags) {
            this.subscribeStates(s);
        }
    }
...
function parseRscpToken(buffer, pos, text) {
    ...
    switch (typeName) {
        case 'None':
            if (len > 0) {
                text.content += `CAUTION: length of data is ${len} `;
            }
            return 7 + len;
        case 'Container':
        ...
        case 'Bool':
            if (
                buffer.readUInt8(pos + 7) > 31 &&
                buffer.readUInt8(pos + 7) < 127 &&
                (typeName == 'Char8' || typeName == 'UChar8')
            ) {
                text.content += `value: ${buffer.toString('utf8', pos + 7, pos + 8)} `;
            } else {
                text.content += `value: 0x${buffer
                    .readUInt8(pos + 7)
                    .toString(16)
                    .toUpperCase()
                    .padStart(2, '0')} `;
        ...
        default:
            if (len > 0) {
                text.content += `${dumpRscpFrame(buffer.slice(pos + 7, pos + 7 + len))} `;
            }
            return 7 + len;
    }
}
