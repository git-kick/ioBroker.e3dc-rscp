...
class E3dcRscp extends utils.Adapter {
	...
	async onReady() {
		...
		var encryptionIV = new Buffer.alloc( BLOCK_SIZE, 0xFF );
		var decryptionIV = new Buffer.alloc( BLOCK_SIZE, 0xFF );
		var aesKey = new Buffer.alloc( KEY_SIZE, 0xFF );
		...
		tcpConnection.on('data', (data) => {
			this.log.info('\nReceived response');
			var decryptedData = Buffer.from(cipher.decrypt(data, 256, decryptionIV))
